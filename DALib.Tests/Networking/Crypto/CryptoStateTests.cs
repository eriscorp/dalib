using DALib.Networking.Crypto;

namespace DALib.Tests.Networking.Crypto;

public class CryptoStateTests
{
    // Salt table - formula output vs known-good literals

    /// <summary>
    ///     Asserts that the formulaic salt-table generator produces byte-identical output
    ///     to the known-good literal arrays at the bottom of this file. If a formula ever
    ///     drifts, this test catches it.
    /// </summary>
    [Theory]
    [InlineData((byte)0)]
    [InlineData((byte)1)]
    [InlineData((byte)2)]
    [InlineData((byte)3)]
    [InlineData((byte)4)]
    [InlineData((byte)5)]
    [InlineData((byte)6)]
    [InlineData((byte)7)]
    [InlineData((byte)8)]
    [InlineData((byte)9)]
    public void SaltTable_AllSeeds_MatchKnownLiteralBytes(byte seed)
    {
        var state = new CryptoState { EncryptionSeed = seed };

        var generated = state.GetSalt().ToArray();
        var literal = KnownSaltLiterals[seed];

        generated.Should().Equal(literal);
    }

    // Opcode -> EncryptMethod tables

    [Theory]
    [InlineData((byte)0x00, EncryptMethod.None)]
    [InlineData((byte)0x10, EncryptMethod.None)]
    [InlineData((byte)0x48, EncryptMethod.None)]
    [InlineData((byte)0x02, EncryptMethod.Normal)]
    [InlineData((byte)0x03, EncryptMethod.Normal)]
    [InlineData((byte)0x39, EncryptMethod.MD5Key)]
    [InlineData((byte)0x3A, EncryptMethod.Normal)]
    [InlineData((byte)0x0E, EncryptMethod.MD5Key)]
    public void GetClientEncryptMethod_KnownOpcodes_ReturnExpected(byte opcode, EncryptMethod expected)
    {
        CryptoState.GetClientEncryptMethod(opcode).Should().Be(expected);
    }

    [Theory]
    [InlineData((byte)0x00, EncryptMethod.None)]
    [InlineData((byte)0x03, EncryptMethod.None)]
    [InlineData((byte)0x40, EncryptMethod.None)]
    [InlineData((byte)0x7E, EncryptMethod.None)]
    [InlineData((byte)0x01, EncryptMethod.Normal)]
    [InlineData((byte)0x02, EncryptMethod.Normal)]
    [InlineData((byte)0x07, EncryptMethod.MD5Key)]
    [InlineData((byte)0x0D, EncryptMethod.MD5Key)]
    public void GetServerEncryptMethod_KnownOpcodes_ReturnExpected(byte opcode, EncryptMethod expected)
    {
        CryptoState.GetServerEncryptMethod(opcode).Should().Be(expected);
    }

    // Key-table generation

    [Fact]
    public void GenerateKeyTable_ProducesExactly1024Bytes()
    {
        var state = new CryptoState();
        state.GenerateKeyTable("Aisling");

        state.EncryptionKeyTable.Length.Should().Be(1024);
    }

    [Fact]
    public void GenerateKeyTable_IsDeterministic()
    {
        var a = new CryptoState();
        var b = new CryptoState();

        a.GenerateKeyTable("Aisling");
        b.GenerateKeyTable("Aisling");

        a.EncryptionKeyTable.Should().Equal(b.EncryptionKeyTable);
    }

    [Fact]
    public void GenerateKeyTable_DifferentNames_ProduceDifferentTables()
    {
        var a = new CryptoState();
        var b = new CryptoState();

        a.GenerateKeyTable("Aisling");
        b.GenerateKeyTable("Mundane");

        a.EncryptionKeyTable.Should().NotEqual(b.EncryptionKeyTable);
    }

    [Fact]
    public void GenerateKey_Returns9Bytes()
    {
        var state = new CryptoState();
        state.GenerateKeyTable("Aisling");

        var key = state.GenerateKey(0x1234, 0x56);

        key.Length.Should().Be(9);
    }

    [Fact]
    public void GenerateKey_IsDeterministic()
    {
        var state = new CryptoState();
        state.GenerateKeyTable("Aisling");

        var first = state.GenerateKey(0x1234, 0x56);
        var second = state.GenerateKey(0x1234, 0x56);

        first.Should().Equal(second);
    }

    // Lobby-response parsing

    [Fact]
    public void UpdateFromLobbyResponse_ExtractsSeedAndKey()
    {
        // Synthetic 0x00 S->C lobby response:
        //   [0] opcode (0x00)
        //   [1] unknown (0x00)
        //   [2..5] serverTableCrc (we don't read this)
        //   [6] seed (0x05)
        //   [7] keyLength (9)
        //   [8..16] key
        var payload = new byte[]
        {
            0x00,                               // opcode
            0x00,                               // unknown
            0xAA, 0xBB, 0xCC, 0xDD,             // serverTableCrc
            0x05,                               // seed
            0x09,                               // keyLength
            0x01, 0x02, 0x03, 0x04, 0x05,       // key bytes
            0x06, 0x07, 0x08, 0x09
        };

        var state = new CryptoState();
        state.UpdateFromLobbyResponse(payload);

        state.EncryptionSeed.Should().Be((byte)0x05);
        state.EncryptionKey.Should().Equal((byte)0x01, (byte)0x02, (byte)0x03, (byte)0x04,
            (byte)0x05, (byte)0x06, (byte)0x07, (byte)0x08, (byte)0x09);
        state.IsInitialized.Should().BeTrue();
    }

    [Fact]
    public void UpdateFromLobbyResponse_OnShortPayload_LeavesStateUntouched()
    {
        var state = new CryptoState();
        state.UpdateFromLobbyResponse(new byte[] { 0x00, 0x00 });

        state.IsInitialized.Should().BeFalse();
    }

    [Fact]
    public void IsInitialized_ReflectsKeyPresence()
    {
        var state = new CryptoState();
        state.IsInitialized.Should().BeFalse();

        state.EncryptionKey = new byte[] { 0x01 };
        state.IsInitialized.Should().BeTrue();
    }

    // Encrypt / decrypt round-trips

    [Fact]
    public void EncryptClientPacket_None_PassesThroughUnchanged()
    {
        var state = MakeReadyState();
        var payload = new byte[] { 0x00, 0xDE, 0xAD, 0xBE, 0xEF }; // opcode 0x00 = None

        var result = state.EncryptClientPacket(0x00, payload);

        result.Should().BeSameAs(payload);
    }

    [Fact]
    public void EncryptClientPacket_Normal_DecryptRoundTrips()
    {
        // Opcode 0x02 is Normal-encrypted C->S.
        var state = MakeReadyState();
        var plain = new byte[] { 0x02, 0xCA, 0xFE, 0xBA, 0xBE, 0x11, 0x22, 0x33 };

        var ordinalUsed = state.ClientOrdinal;
        var encrypted = state.EncryptClientPacket(0x02, plain);

        // encrypted = [opcode][ordinal][encrypted data][7-byte footer]
        encrypted[0].Should().Be((byte)0x02);
        encrypted[1].Should().Be(ordinalUsed);

        // Slice off [opcode][ordinal] to get what DecryptClient expects.
        var decryptInput = encrypted.AsSpan(2).ToArray();
        var decrypted = state.DecryptClient(0x02, ordinalUsed, decryptInput);

        // Normal inner padding: trailing 0x00.
        decrypted.Should().Equal((byte)0xCA, (byte)0xFE, (byte)0xBA, (byte)0xBE,
            (byte)0x11, (byte)0x22, (byte)0x33, (byte)0x00);
    }

    [Fact]
    public void EncryptClientPacket_MD5Key_DecryptRoundTrips()
    {
        // Opcode 0x0E (Chat) is MD5Key-encrypted C->S.
        var state = MakeReadyState();
        var plain = new byte[] { 0x0E, 0xCA, 0xFE, 0xBA, 0xBE };

        var ordinalUsed = state.ClientOrdinal;
        var encrypted = state.EncryptClientPacket(0x0E, plain);

        var decryptInput = encrypted.AsSpan(2).ToArray();
        var decrypted = state.DecryptClient(0x0E, ordinalUsed, decryptInput);

        // MD5Key inner padding: trailing 0x00 + opcode copy.
        decrypted.Should().Equal((byte)0xCA, (byte)0xFE, (byte)0xBA, (byte)0xBE,
            (byte)0x00, (byte)0x0E);
    }

    [Fact]
    public void EncryptServerPacket_None_PassesThroughUnchanged()
    {
        var state = MakeReadyState();
        var payload = new byte[] { 0x00, 0xDE, 0xAD };

        var result = state.EncryptServerPacket(0x00, payload);

        result.Should().BeSameAs(payload);
    }

    [Fact]
    public void EncryptServerPacket_Normal_DecryptRoundTrips()
    {
        // Opcode 0x02 is Normal-encrypted S->C.
        var state = MakeReadyState();
        var plain = new byte[] { 0x02, 0xCA, 0xFE, 0xBA, 0xBE };

        var ordinalUsed = state.ServerOrdinal;
        var encrypted = state.EncryptServerPacket(0x02, plain);

        var decryptInput = encrypted.AsSpan(2).ToArray();
        var decrypted = state.DecryptServer(0x02, ordinalUsed, decryptInput);

        // S->C has no inner padding.
        decrypted.Should().Equal((byte)0xCA, (byte)0xFE, (byte)0xBA, (byte)0xBE);
    }

    [Fact]
    public void EncryptServerPacket_MD5Key_DecryptRoundTrips()
    {
        // Opcode 0x07 (DrawObjects) is MD5Key-encrypted S->C.
        var state = MakeReadyState();
        var plain = new byte[] { 0x07, 0x11, 0x22, 0x33, 0x44 };

        var ordinalUsed = state.ServerOrdinal;
        var encrypted = state.EncryptServerPacket(0x07, plain);

        var decryptInput = encrypted.AsSpan(2).ToArray();
        var decrypted = state.DecryptServer(0x07, ordinalUsed, decryptInput);

        decrypted.Should().Equal((byte)0x11, (byte)0x22, (byte)0x33, (byte)0x44);
    }

    [Fact]
    public void EncryptClientPacket_IncrementsClientOrdinal()
    {
        var state = MakeReadyState();
        state.ClientOrdinal = 5;

        state.EncryptClientPacket(0x0E, new byte[] { 0x0E, 0x00 });

        state.ClientOrdinal.Should().Be((byte)6);
    }

    [Fact]
    public void EncryptServerPacket_IncrementsServerOrdinal()
    {
        var state = MakeReadyState();
        state.ServerOrdinal = 5;

        state.EncryptServerPacket(0x07, new byte[] { 0x07, 0x00 });

        state.ServerOrdinal.Should().Be((byte)6);
    }

    // Helpers

    private static CryptoState MakeReadyState()
    {
        var state = new CryptoState
        {
            EncryptionSeed = 5,
            EncryptionKey = new byte[] { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09 },
        };
        state.GenerateKeyTable("Aisling");

        return state;
    }

    // Known-good salt literals
    //
    // Ground-truth literal arrays for SaltTable_AllSeeds_MatchKnownLiteralBytes;
    // production code uses the formulaic GenerateSalt() instead.

    private static readonly byte[][] KnownSaltLiterals =
    [
        // Seed 0
        [
            
        ],
        // Seed 1
        [
            
        ],
        // Seed 2
        [
            
        ],
        // Seed 3
        [
            
        ],
        // Seed 4
        [
            
        ],
        // Seed 5
        [
            
        ],
        // Seed 6
        [
            
        ],
        // Seed 7
        [
            
        ],
        // Seed 8
        [
            
        ],
        // Seed 9
        [
            
        ]
    ];
}
